...
<div class="row">
    <div class="col-lg-12">
        <h1>课程列表 <small>课程管理总览(课程和老师是一一对应关系，先有老师的课程，再允许添加课程列表)</small></h1>
    </div>
</div><!-- /.row -->
<div class="row">
    <div class="col-lg-12">
            ...
            <table class="table table-bordered table-hover tablesorter">
                ...
                <tbody>
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p_d): $mod = ($i % 2 );++$i;?><tr>
                            <td><?php echo ($p_d['class_name']); ?></td>
                            <td><?php echo ($p_d['class_teacher']); ?></td>
                            <td><?php echo ($p_d['class_price']); ?></td>
                            <td><?php echo (date("Y-m-d H:i:s",$p_d['class_time'])); ?></td>
                            <td>
                                <a href="<?php echo U('pro_detail','','');?>/id/<?php echo ($p_d['class_id']); ?>" this-id="<?php echo ($p_d['class_id']); ?>" class="btn btn-primary">查看详情</a>
                                <button this-id="<?php echo ($p_d['class_id']); ?>" onclick="ask_delete(this)" class="btn btn-danger">删除</button>
                                <a href="<?php echo U('edit_pro','','');?>/id/<?php echo ($p_d['class_id']); ?>" class="btn btn-success">编辑</a>
                            </td>
...
<script language="JavaScript">
    function ask_delete(e){
        if(confirm("是否要删除这门课程？")){
            window.location.href="<?php echo U('Admin/Public/del','','');?>/table/tb_class/pk/class_id/id/"+$(e).attr("this-id");
        }
    }
</script>
    </div>
