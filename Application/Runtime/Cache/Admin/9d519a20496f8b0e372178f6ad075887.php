...
<div id="wrapper">
    ...
    <div class="col-md-12">
        <ul class="nav nav-pills">
            ...
            <li>
                <a href="<?php echo U('Admin/Index/index_img','','');?>">大图管理首页</a>
            </li>
            <!--<li>-->
                <!--<a href="<?php echo ($U['news_list']); ?>">新闻列表</a>-->
            <!--</li>-->
            <li>
                <a href="<?php echo U('Admin/Index/add_index_img','','');?>">发布图片</a>
            </li>
        </ul>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
        <h1>首页大图列表 <small>首页大图管理总览</small></h1>
    </div>
</div><!-- /.row -->
<div class="row">
    <div class="col-lg-12">
        <div class="table-responsive">
            <table class="table table-bordered table-hover tablesorter">
                ...
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p_d): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($p_d['img_title']); ?></td>
                        <?php if($p_d['is_show'] == 1): ?><td style="background-color: #b16549">展示中</td>
                            <?php else: ?>
                            <td>不显示</td><?php endif; ?>
                        <td>
                            <button this-id="<?php echo ($p_d['img_id']); ?>" onclick="ask_delete(this)" class="btn btn-danger">删除</button>
                            <a href="<?php echo U('Admin/Index/edit_index_img','','');?>/id/<?php echo ($p_d['img_id']); ?>" class="btn btn-success">编辑</a>
                        <?php if($p_d['is_show'] == 1): ?><a href="<?php echo U('Admin/Public/xiajia','','');?>/table/tb_img/pk/img_id/id/<?php echo ($p_d['img_id']); ?>" class="btn btn-danger">隐藏</a>
                        <?php else: ?>
                                <a href="<?php echo U('Admin/Public/shangjia','','');?>/table/tb_img/pk/img_id/id/<?php echo ($p_d['img_id']); ?>" class="btn btn-default">展示</a><?php endif; ?>
                        </td>
...
<script language="JavaScript">
    function ask_delete(e){
        if(confirm("是否要删除该条图片？")){
            window.location.href="<?php echo U('Admin/Public/del','','');?>/table/tb_img/pk/img_id/id/"+$(e).attr("this-id");
        }
    }
</script>
    </div>
