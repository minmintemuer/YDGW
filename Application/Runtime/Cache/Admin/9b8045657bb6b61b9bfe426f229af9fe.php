...
<div id="wrapper">
    ...
    <div class="col-md-12">
        <ul class="nav nav-pills">
            ...
            <li>
                <a href="<?php echo U('Admin/Index/kefu_','','html');?>">客服管理首页</a>
            </li>
        </ul>
    </div>
</div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover tablesorter">
                ...
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p_d): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($p_d['customer_service_name']); ?></td>
                        <td><?php echo ($p_d['customer_service_tel']); ?></td>
                        <td><?php echo ($p_d['customer_service_user']); ?></td>
                        <?php if($p_d['is_use'] == 1): ?><td>使用中</td>
                            <?php else: ?>
                            <td style="background-color: #b16549">已禁用</td><?php endif; ?>
                        <?php if($p_d['is_shenhe'] == 1): ?><td>已审核</td>
                            <?php else: ?>
                            <td style="background-color: #b16549">未审核</td><?php endif; ?>
                        <td>
                            <a href="<?php echo U('Admin/Index/kefu_detail','','');?>/id/<?php echo ($p_d['customer_service_id']); ?>" this-id="<?php echo ($p_d['customer_service_id']); ?>" class="btn btn-primary">查看该客服详情</a>
                            <button this-id="<?php echo ($p_d['customer_service_id']); ?>" onclick="ask_delete(this)" class="btn btn-danger">删除该客服</button>
                            <?php if($p_d['is_use'] == 1): ?><a href="<?php echo U('Admin/Public/unUseKefu','','');?>/table/tb_customer_service/pk/customer_service_id/id/<?php echo ($p_d['customer_service_id']); ?>" class="btn btn-danger">禁用该客服</a>
                                <?php else: ?>
                                <a href="<?php echo U('Admin/Public/useKefu','','');?>/id/<?php echo ($p_d['customer_service_id']); ?>" class="btn btn-success">启用该客服</a><?php endif; ?>
                            <?php if($p_d['is_shenhe'] == 0): ?><a href="<?php echo U('Admin/Public/shenhe','','');?>/id/<?php echo ($p_d['customer_service_id']); ?>" class="btn btn-success">审核通过</a><?php endif; ?>
                        </td>
...
<script language="JavaScript">
    function ask_delete(e){
        if(confirm("是否要删除该客服？注意：该操作不可逆，请谨慎操作")){
            window.location.href="<?php echo U('Admin/Public/del','','');?>/table/tb_customer_service/pk/customer_service_id/id/"+$(e).attr("this-id");
        }
    }
</script>
    </div>
