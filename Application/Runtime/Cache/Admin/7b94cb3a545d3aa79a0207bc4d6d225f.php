...
<div class="row">
    <div class="col-md-12">
        <div class="form-group">
            <label>鸡汤标题</label>
            <input class="form-control" id="soft_text_title" value="<?php echo ($data[0]['soft_text_title']); ?>">
            <p class="help-block">请输入鸡汤标题</p>
        </div>
        <div class="form-group">
            <label>鸡汤作者</label>
            <input class="form-control" id="sofe_text_admin" value="<?php echo ($data[0]['sofe_text_admin']); ?>">
            <p class="help-block">请输入鸡汤更新人</p>
        </div>
        <div class="form-group">
            <label>鸡汤展示图片</label>
            <form id="image_form" action="<?php echo U('/Admin/Ajax/ajax_upload_image','','');?>" method="post" target="hidden_frame" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <div class="col-md-3" style="display: block">
                        <img  id="image_area" src="/company/YDGW/Public<?php echo ($data[0]['soft_text_img']); ?>" width="100%" height="auto" ihref="<?php echo ($data[0]['soft_text_img']); ?>" >
                    </div>
                </div>
            </form> <iframe style="display:none" name='hidden_frame' id="hidden_frame"></iframe>
            <p class="help-block">请上传鸡汤图片</p>
...
<script language="JavaScript">
    function tijiao(){
        if(confirm("是否提交？")){
            $.ajax({
                url:"<?php echo U('/Admin/Ajax/ajax_save_soft_text','','html');?>",
                dataType: 'json',
                type:'post',
                data:{
                    id:<?php echo ($data[0]['soft_text_id']); ?>,
