...
<div class="row">
    <div class="col-lg-12">
        <h1>修改活动 <small><?php echo ($data[0]['activety_name']); ?></small></h1>
    </div>
</div><!-- /.row -->
<div class="row">
    <div class="col-md-12">
        <div class="form-group">
            <label>活动名称</label>
            <input class="form-control" id="activety_name" value="<?php echo ($data[0]['activety_name']); ?>">
            <p class="help-block">请输入活动名称</p>
        </div>
        <div class="form-group">
            <label>操作人员</label>
            <input class="form-control" id="activety_admin" value="<?php echo ($data[0]['activety_admin']); ?>">
            <p class="help-block">请输入活动名称</p>
        </div>
        <div class="form-group">
            <label>面向人群</label>
            <input class="form-control" id="activety_crowd" value="<?php echo ($data[0]['activety_crowd']); ?>">
            <p class="help-block">请输入面向人群</p>
        </div>
        <div class="form-group">
            <label>开始时间</label>
            <input class="form-control" id="activety_start_time" value="<?php echo (date("Y-m-d",$data[0]['activety_start_time'])); ?>" placeholder="格式：YYYY-MM-DD 例如：2017-03-20">
            <p class="help-block" >请输入活动开始时间</p>
        </div>
        <div class="form-group">
            <label>结束时间</label>
            <input class="form-control" id="activety_end_time" value="<?php echo (date("Y-m-d",$data[0]['activety_end_time'])); ?>" placeholder="格式：YYYY-MM-DD 例如：2017-03-20">
            <p class="help-block">请输入活动结束时间</p>
        </div>
        <div class="form-group">
            <label>活动展示图片</label>
            <form id="image_form" action="<?php echo U('/Admin/Ajax/ajax_upload_image','','');?>" method="post" target="hidden_frame" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <div class="col-md-3" style="display: block">
                        <img  id="image_area" src="/company/YDGW/Public<?php echo ($data[0]['activety_img']); ?>" width="100%" height="auto" ihref="<?php echo ($data[0]['activety_img']); ?>" >
                    </div>
                </div>
...
<script language="JavaScript">
    function tijiao(){
        if(confirm("是否提交？")){
            $.ajax({
                url:"<?php echo U('/Admin/Ajax/ajax_save_zhuanti','','html');?>",
                dataType: 'json',
                type:'post',
                data:{
                    id:<?php echo ($data[0]['activety_id']); ?>,
