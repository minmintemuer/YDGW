        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover tablesorter">
                ...
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p_d): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($p_d['introduction_id']); ?></td>
                        <?php if($p_d['is_show'] == 1): ?><td style="background-color: #b16549">展示中</td>
                            <?php else: ?>
                            <td>不显示</td><?php endif; ?>
                        <td><?php echo (Date("Y-m-d H:i:s",$p_d['introduction_time'])); ?></td>
                        <td>
                            <a href="<?php echo U('Admin/Index/jianjie_detail','','');?>/id/<?php echo ($p_d['introduction_id']); ?>" this-id="<?php echo ($p_d['introduction_id']); ?>" class="btn btn-primary">查看详情</a>
                            <button this-id="<?php echo ($p_d['introduction_id']); ?>" onclick="ask_delete(this)" class="btn btn-danger">删除</button>
                            <a href="<?php echo U('Admin/Index/edit_jianjie','','');?>/id/<?php echo ($p_d['introduction_id']); ?>" class="btn btn-success">编辑</a>
                            <?php if($p_d['is_show'] == 1): ?><a href="<?php echo U('Admin/Public/xiajianjie','','');?>/table/tb_introduction/pk/introduction_id/id/<?php echo ($p_d['introduction_id']); ?>" class="btn btn-danger">隐藏</a>
                                <?php else: ?>
                                <a href="<?php echo U('Admin/Public/shangjiejian','','');?>/table/tb_introduction/pk/introduction_id/id/<?php echo ($p_d['introduction_id']); ?>" class="btn btn-default">展示</a><?php endif; ?>
                        </td>
...
<script language="JavaScript">
    function ask_delete(e){
        if(confirm("是否要删除该条简介？")){
            window.location.href="<?php echo U('Admin/Public/del','','');?>/table/tb_introduction/pk/introduction_id/id/"+$(e).attr("this-id");
        }
    }
</script>
    </div>
