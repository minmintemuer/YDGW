...
<div class="row">
    <div class="col-md-12">
        <div class="form-group">
            <label>网页关键字</label>
            <input class="form-control" id="seo_keyword" value="<?php echo ($data[0]['seo_keyword']); ?>">
            <p class="help-block">请输入网页关键字</p>
        </div>
        <div class="form-group">
            <label>网页作者</label>
            <input class="form-control" id="seo_author" value="<?php echo ($data[0]['seo_author']); ?>">
            <p class="help-block">请输入网页作者</p>
        </div>
        <div class="form-group">
            <label>网页摘要</label>
            <input class="form-control" id="seo_description" value="<?php echo ($data[0]['seo_description']); ?>">
            <p class="help-block">请输入网页摘要</p>
        </div>
        <div class="form-group">
            <label>直播入口</label>
            <input class="form-control" id="seo_live_entry" value="<?php echo ($data[0]['seo_live_entry']); ?>">
            <p class="help-block">请输入直播地址</p>
        </div>
        <div class="form-group">
            <label>在线客服qq</label>
            <input class="form-control" id="seo_online_qq" value="<?php echo ($data[0]['seo_online_qq']); ?>">
            ...
            <form id="image_form" action="<?php echo U('/Admin/Ajax/ajax_upload_image','','');?>" method="post" target="hidden_frame" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <div class="col-md-3" style="display: block">
                        <img  id="image_area" src="/company/YD/Public<?php echo ($data[0]['seo_wechat']); ?>" width="100%" height="auto" ihref="<?php echo ($data[0]['seo_wechat']); ?>" >
                    </div>
                </div>
            </form>
            ...
            <script language="JavaScript">
                ...
                function callback(message,success)
                {
                    if(success==false)
                    {
                        ...
                        $("#error_info").hide();
                        $("#image_area").attr("src",'/company/YD/Public'+message);
...
<script language="JavaScript">
    function tijiao(){
        if(confirm("是否提交？")){
            $.ajax({
                url:"<?php echo U('/Admin/Ajax/ajax_add_seo','','');?>",
                dataType: 'json',
                type:'post',
                data:{
